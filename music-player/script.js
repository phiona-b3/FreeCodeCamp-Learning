//accessing ids using the getElementById() method
//getElementById() method accepts id as a string without the #
...
//the array which contain objects
//objects are declared using the { } brackets
const allSongs = [
  {
    id: 0,
...
//"userData" object will contain the songs, current song playing and the time of hte current song
//Since users will be able to shuffle and delete songs from the playlist, you will need to create a copy of the allSongs
//array without mutating the original. This is where the spread operator comes in handy.
//he spread operator (...) allows you to copy all elements from one array into another. It can also be used to concatenate multiple arrays into one.
...
//find() method retrieves the first element within an array that fulfills the conditions specified in the provided callback function.
// if no element satisfies the condition, the method returns undefined
//the classList property and the add() method to add sthe playing class to the playButton element. This will look for the class playing in the CSS file and add it to the playButton element.
...
//helpful when you want to create a new array based on the values of an existing array
...
//the join() method is used to concatenate all the elements of an array into a single strring
//it takes an optional parameter called separator which is used to separate each element of the array
...
//set up an event listener which will detect when the currently playing song ends.
